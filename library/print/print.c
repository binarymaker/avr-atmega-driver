                     ...
                     bool_et isZeroPadding_b)
{
  ...
  if ((num < 0) && (radix == RADIX_DEC))
  {
    num = -num;
    is_negative = 1;
    if (zeropad_length && (TRUE == isZeroPadding_b))
    {
      zeropad_length--;
    }
  }
  ...
  for (ptr = argList; *ptr != '\0'; ptr++)
  {
    ...
    if (ch == '%') /* Check for '%' for format specifier */
    {
      ...
      if ((ch >= '0') && (ch <= '9'))
      {
        if (ch == '0')
        {
          isZeroPadding_b = TRUE;
          ptr++;
          ch = *ptr;
        }
        else
        {
          isZeroPadding_b = FALSE;
        }
